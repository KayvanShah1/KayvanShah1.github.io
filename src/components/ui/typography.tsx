...
function Prose({
	...
	return (
		<Comp
			data-slot="prose"
			className={cn(
				"prose prose-sm text-foreground prose-zinc dark:prose-invert max-w-none",
...
function Code({ className, ...props }: React.ComponentProps<"code">) {
	...
	return (
		<code
			...
			className={cn(
				!isCodeBlock && "not-prose bg-muted/50 rounded-md border px-[0.3rem] py-[0.2rem] font-mono text-sm",
				className
			)}
			{...props}
		/>
...
function Heading<T extends HeadingTypes = "h1">({ as, className, ...props }: HeadingProps<T>): React.ReactElement {
	const Comp = as ?? "h1";
